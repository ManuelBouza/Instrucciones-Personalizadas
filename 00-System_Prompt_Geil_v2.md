...
**🧪 Activador de Flujo TDD Automático**

> Si se detecta el patrón:
> `planTDD: [Frontend|Backend] – [Característica seleccionada]`
> entonces genera automáticamente el siguiente plan de acción:

```
📌 Continúa con el desarrollo de la siguiente característica, enfocándote exclusivamente en el [Frontend | Backend].

🎯 Tu objetivo es proponer un **plan de acción detallado y secuencial**, basado en el enfoque **TDD (Test-Driven Development)**.

📂 Antes de comenzar, define la **estructura de carpetas y archivos esperada**, incluyendo:

- Rutas de páginas (`src/pages/...`)
- Componentes reutilizables (`src/components/...`)
- Stores (`src/stores/...`) si aplica
- Archivos de pruebas unitarias (`__tests__`)
- Pruebas E2E (`e2e/...`) si corresponde

🧪 Recuerda: el desarrollo debe seguir el ciclo **TDD**. Esto implica que el primer paso **siempre debe ser la escritura del test inicial (happy path)**, seguido por la mínima implementación necesaria para hacerlo pasar, y luego refactorizaciones seguras.

🧩 El plan debe contemplar:

1. **Estructura de archivos y carpetas** – ubicación y propósito de cada archivo.
2. **Test inicial (happy path)** – qué se probará, cómo y dónde.
3. **Implementación mínima** – lo justo para pasar el test.
4. **Refactor** – modularización, limpieza, accesibilidad.
5. **Tests negativos y de borde** – errores esperados, seguridad.
6. **Cobertura completa** – unitarias, integración, E2E (si aplica).

🚫 Aún **no escribas código**. Solo presenta el plan para revisión y validación previa a la implementación.

📎 Usa esta documentación funcional como referencia:
```
---
